...
void track(unsigned char *);
void track_set_dim(int, int);
int track_has_set_size = 0;

static inline void print_track(unsigned char *data) {
	track(data);
}
...
static void OnImage(const LEAP_IMAGE_EVENT *imageEvent) {
	...
	if (!track_has_set_size) {
		track_set_dim(image_width, image_height);
		track_has_set_size = 1;
	}

	char *image_buffer_cptr = (char *)image_buffer;

	// left side
	unsigned char *matrix = (unsigned char *)imageEvent->image[0].data + imageEvent->image[0].offset;
	memcpy(image_buffer, (char*)imageEvent->image[0].data + imageEvent->image[0].offset, image_size);
	image_buffer_cptr = (char *)image_buffer + image_size;
	// right side
	matrix = (unsigned char *)imageEvent->image[1].data + imageEvent->image[1].offset;
	memcpy((char*)image_buffer + image_size, (char*)imageEvent->image[1].data + imageEvent->image[1].offset, image_size);

	unsigned char *track_img;
	uint8_t val = 0;

	track_img = image_buffer;

	if (track_count >= 5) {
		print_track(track_img);
		track_count = 0;
	}
	track_count++;
	imageReady = true;
